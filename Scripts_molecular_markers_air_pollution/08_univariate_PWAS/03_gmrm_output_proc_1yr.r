...
#Read in step 1 output files
file_list <- list.files(path = "/1yr/output_1yr", recursive = TRUE, pattern = "*.csv", full.names = TRUE)
results_list <- list()
...
fwrite(heritability, file = "/1yr/pollutant_herit_burnin.csv")

#write out full results
fwrite(results_df, file = "/1yr/step1_full_results.csv")
...
file_list <- list.files(path = "/1yr/output_1yr", recursive = TRUE, pattern = "*.mlma", full.names = TRUE)
results_list <- list()
...
#add in proteins
protein_order <- fread("/input_1yr/protein_order.csv")
...
results_df <- bind_rows(results_list, .id = "pollutant")
head(results_df)
fwrite(results_df, file = "/1yr/full_results_step2.csv")
...
#write these out:
fwrite(Cpg_counts, file = "/1yr/Cpg_counts.csv")
fwrite(protein_counts, file = "/1yr/pollutant_counts.csv")
fwrite(highpip, file ="/1yr/highpipcpgs.csv")
