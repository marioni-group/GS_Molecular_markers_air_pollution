...
coxdf2 %>% filter(status_htn == "1" & event_htn < appt2) %>% dim() #7 cases where diagnosis in same month as baseline appointment -> set these as prevalent cases later
coxdf3 <- coxdf2
...
coxdf4 <- coxdf4 %>% 
  mutate(hypertension = ifelse(tte_htn_years < 0, 1, hypertension)) #if negative tte (6 cases where diagnosis in month of gs appt ->set to prevalent) 
table(coxdf4$hypertension) 
