...
file_list <- list.files(path = "/6month/output_6month", recursive = TRUE, pattern = "*.csv", full.names = TRUE)
results_list <- list()
...
fwrite(heritability, file = "/6month/pollutant_herit_burnin.csv")

#write out full results
fwrite(results_df, file = "/6month/step1_full_results.csv")
...
file_list <- list.files(path = "/6month/output_6month", recursive = TRUE, pattern = "*.mlma", full.names = TRUE)
results_list <- list()
...
results_df <- bind_rows(results_list, .id = "pollutant")
head(results_df)
fwrite(results_df, file = "/6month/full_results_step2.csv")
...
#write these out:
fwrite(protein_counts, file = "/6month/Cpg_counts.csv")
fwrite(pollutant_counts, file = "/6month/pollutant_counts.csv")
fwrite(highpip, file ="/6month/highpipcpgs.csv")
...
summary_df
fwrite(summary_df, file = "/6month/summary_df.csv")
